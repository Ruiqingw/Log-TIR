...
for name in \
  ...
  GRPO_AGENT_FUNC \
  GRPO_MULTI_TURN \
  LOGTIR_AGENT_MAX_TURNS \
  LOGTIR_AGENT_TIMEOUT \
  LOGTIR_AGENT_TURN_PENALTY \
  GRPO_GPUS_PER_NODE \
  GRPO_RAY_NUM_GPUS \
  GRPO_ACTOR_GPUS_PER_NODE \
  GRPO_REF_GPUS_PER_NODE \
  GRPO_VLLM_ENGINES \
  GRPO_VLLM_TP \
  GRPO_VLLM_ENFORCE_EAGER \
  GRPO_VLLM_ENABLE_SLEEP \
  GRPO_VLLM_GPU_MEMORY_UTILIZATION \
  GRPO_COLOCATE_ALL \
  GRPO_TRAIN_BATCH_SIZE \
  GRPO_ROLLOUT_BATCH_SIZE \
  GRPO_N_SAMPLES_PER_PROMPT \
  GRPO_ROLLOUT_TEMPERATURE \
  GRPO_ROLLOUT_TOP_P \
  GRPO_DYNAMIC_BATCH_ENABLE \
  GRPO_MAX_EPOCHS \
  GRPO_MAX_SAMPLES \
  GRPO_MAX_LEN \
  GRPO_MAX_NEW_TOKENS \
  GRPO_PACKING_SAMPLES \
  GRPO_ADAM_OFFLOAD \
  GRPO_ACTOR_LR \
  GRPO_ACTOR_ENTROPY_COEF \
  GRPO_INIT_KL_COEF \
  GRPO_USE_KL_LOSS \
  GRPO_KL_ESTIMATOR \
  GRPO_EVAL_DATA \
  ...
  LOGTIR_RAY_TMPDIR \
  LOGTIR_RAY_NUM_CPUS \
  LOGTIR_ENABLE_RUN_LOGS \
  LOGTIR_WANDB \
  LOGTIR_WANDB_PREFLIGHT \
  LOGTIR_WANDB_PREFLIGHT_TIMEOUT \
  LOGTIR_WANDB_PREFLIGHT_STRICT \
  WANDB_MODE \
  WANDB_PROJECT \
  WANDB_RUN_NAME \
  WANDB_API_KEY \
  START_RAY; do
  if [[ ${!name+x} ]]; then
    LOGTIR_CONFIG_OVERRIDES+=("$name=${!name}")
  fi
done
...
for assignment in "${LOGTIR_CONFIG_OVERRIDES[@]}"; do
  export "$assignment"
done
...
GRPO_ROLLOUT_TEMPERATURE="${GRPO_ROLLOUT_TEMPERATURE:-}"
GRPO_ROLLOUT_TOP_P="${GRPO_ROLLOUT_TOP_P:-}"
...
if [[ -n "$GRPO_ROLLOUT_TEMPERATURE" ]]; then
  GRPO_COMMAND+=(--rollout.temperature "$GRPO_ROLLOUT_TEMPERATURE")
fi
if [[ -n "$GRPO_ROLLOUT_TOP_P" ]]; then
  GRPO_COMMAND+=(--rollout.top_p "$GRPO_ROLLOUT_TOP_P")
fi
